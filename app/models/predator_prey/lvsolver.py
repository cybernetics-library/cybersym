...
# 
values  = linspace(0.3, 0.9, 3)                          # position of X0 between X_f0 and X_f1
...
for v, col in zip(values, vcolors): 
    ...
    p.plot( X[:,0], X[:,1], lw=1.0, color='black', label='X0=(%.f, %.f)' % ( X0[0], X0[1]) )
...
print [x for x in where(Z2 > 10)]

f3 = p.figure()
CS2 = p.contour(X2, Y2, Z2, colors='black', linewidths=2. )
p.ylim(1, ymax)
p.xlim(1, xmax)
f3.savefig('rabbits_and_foxes_3.svg')
#p.show()
